...
void	      gdk_window_move	     (GdkWindow	   *window,
				      gint	    x,
				      gint	    y);
void          gdk_window_clear_intersect_children(GdkWindow *window, 
					GdkRectangle *rect);
void	      gdk_window_offset_children (GdkWindow *window,
             			      gint       offx,
		                      gint       offy);
...
gboolean gdk_font_equal	    (const GdkFont  *fonta,
			     const GdkFont  *fontb);
guchar * gdk_width_array(const GdkFont *font);
...
GdkRegion*    gdk_regions_xor		  (GdkRegion	  *source1,
					   GdkRegion	  *source2);
void 		gdk_regions_union_offset (GdkRegion      *src_dest,
					GdkRegion      *source2,
					int offx,int offy);
GdkRegion* 	gdk_region_tile(GdkRegion *tile, int aX0, int aX1,
					int aY0, int aY1, 
					int src_width, int dest_width);
int 		gdk_region_is_empty(GdkRegion *region);
void 		gdk_region_dump (GdkRegion      *region);
