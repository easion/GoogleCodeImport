...
#elif defined(USE_GI)
	if (i) {
	//printf("fltk:hide xid %x\n", i->xid);
      //if (!parent()) XWithdrawWindow(xdisplay, i->xid, xscreen);
      gi_hide_window( i->xid);
    }
#elif defined(_WIN32)
    if (i) {
...
#elif defined(USE_GI)
	 gi_cliprect_t R;
    R.x = r.x(); R.y = r.y(); R.w = r.w(); R.h = r.h();
    XUnionRectWithRegion(&R, region, region);
#elif defined(_WIN32)
...
void Window::destroy() {
 ...
 stop_drawing(x->xid);
  if (x->region) XDestroyRegion(x->region);
  if (x->frontbuffer) gi_destroy_window( x->frontbuffer);
  gi_destroy_window( x->xid);
#if 0
  if (fltk::gc && fltk::gc->winid == x->xid)
  {
	  gi_destroy_gc(fltk::gc);
     fltk::gc = NULL;
  }
#endif
