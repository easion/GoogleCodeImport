...
void RazorAppSwitcher::AppSwitcher::handleApps()
{
    ...
    QList<gi_window_id_t> l = xfitMan().getClientList();
    QList<gi_window_id_t> merge;
    // setup already used windows
    foreach (gi_window_id_t w, m_orderedWindows)
    {
        if (l.contains(w))
        {
            merge.append(w);
    ...
    // setup new windows
    foreach (gi_window_id_t w, merge)
    {
...
bool RazorAppSwitcher::AppSwitcher::handleEvent(gi_msg_t * e)
{
    if (e->type == GI_MSG_PROPERTYNOTIFY)
    {
        if (!this)
            return false;

        gi_window_id_t w = xfitMan().getActiveWindow();
...
RazorAppSwitcher::SwitcherItem::SwitcherItem(gi_window_id_t window, const QString & text,
