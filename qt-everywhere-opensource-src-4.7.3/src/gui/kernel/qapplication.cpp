...
void QApplicationPrivate::dispatchEnterLeave(QWidget* enter, QWidget* leave) {
    ...
    if (enterOnAlien) {
...
#endif
        {
            ...
            qt_gix_enforce_cursor(cursorWidget, true);
#elif defined(Q_OS_SYMBIAN)
...
{
    return qApp->d_func()->cursor_list.isEmpty() ? 
		0 : &qApp->d_func()->cursor_list.first();
}
 ...
 *****************************************************************************/
#ifndef QT_NO_SESSIONMANAGER
#if defined(Q_WS_WIN) || defined(Q_WS_MAC) || defined(Q_WS_QWS) || defined(Q_WS_GIX)
