  ...
  : NSPlate(down), _bar(up), listener(nsl), listenerArg(nslArg)
{
  ...
  height(h);
  selectInput(GI_MASK_BUTTON_DOWN | GI_MASK_BUTTON_UP | GI_MASK_MOUSE_MOVE | GI_MASK_CONFIGURENOTIFY|GI_MASK_CLIENT_MSG);
...
void NSVScrollbar::dispatchEvent(const gi_msg_t& ev)
{
  switch (ev.type) {
...
void NSHScrollbar::dispatchEvent(const gi_msg_t& ev)
{
  switch (ev.type) {
  case GI_MSG_BUTTON_DOWN:
    {
  ...
  case GI_MSG_MOUSE_MOVE:
	...
	/*case GI_MSG_CLIENT_MSG:
            {
        if(ev.client.client_type==GA_WM_PROTOCOLS
          &&ev.params[0] == GA_WM_DELETE_WINDOW){        
                exit(1);
        }
      }
      break;
