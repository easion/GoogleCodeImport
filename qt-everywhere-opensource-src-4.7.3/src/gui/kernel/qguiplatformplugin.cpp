...
*/
#if 0
		for (QStringList::iterator it = availableStyles.begin();
			it != availableStyles.end(); ++it) { /* C++ STL-style iteration */
			QString current = *it;   /* pointer-style dereference */
			qDebug() << "style: [[" << current << "]]";
		}
#endif

        // Set QGtkStyle for GNOME if available
        QString  gtkStyleKey = QString::fromLatin1("Plastique");
        QString  cleanlooksStyleKey= QString::fromLatin1("cleanlooks");
        if (availableStyles.contains(gtkStyleKey)) {
            stylename = gtkStyleKey;
        }
		else if (availableStyles.contains(cleanlooksStyleKey)) {
			stylename = cleanlooksStyleKey;
		}
		else
			stylename = QLatin1String("windows");
		return stylename;
		// default style for Gix    
...
#elif defined(Q_WS_GIX)
	result = QString::fromLatin1("gix");
#ifndef QT_NO_STYLE_GTK
	result = QGtkStylePrivate::getGConfString(
		QLatin1String("/desktop/gnome/interface/icon_theme"), result);
#endif
