    ...
    : DesktopPlugin(configId, config)
{
    ...
    if (! finalPixmap.isEmpty())
    {
		...
		gi_window_id_t pix=0;//p = pixmap.handle();
		gi_gc_ptr_t gc;

        QPixmap pixmap(finalPixmap);
        pixmap = pixmap.scaled(width,height);
		gi_image_t *img = pixmap.toGixBitmap();
        
		pix = gi_create_pixmap_window(GI_DESKTOP_WINDOW_ID, width, height, gi_screen_format());
        //XGrabServer(QX11Info::display());
		gc = gi_create_gc(pix,NULL);
		gi_draw_image(gc,img,0,0);
        gi_change_property( GI_DESKTOP_WINDOW_ID, 
			XfitMan::atom("_XROOTPMAP_ID"), GA_PIXMAP, 32, G_PROP_MODE_Replace, (unsigned char *) &pix, 1);
        gi_change_property( GI_DESKTOP_WINDOW_ID, 
			XfitMan::atom("ESETROOT_PMAP_ID"), GA_PIXMAP, 32, G_PROP_MODE_Replace, (unsigned char *) &pix, 1);
        //XSetCloseDownMode( RetainPermanent);
        gi_set_window_pixmap(GI_DESKTOP_WINDOW_ID, pix,0);
        gi_clear_window( GI_DESKTOP_WINDOW_ID,FALSE);
		gi_destroy_gc(gc);
		gi_destroy_window(pix);
		gi_destroy_image(img);
        //XUngrabServer(QX11Info::display());
        //XFlush(QX11Info::display());
    }
	else{
		printf("not set window pximap\n");
	}
