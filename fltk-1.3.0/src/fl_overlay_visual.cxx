...
gi_screen_info_t *fl_find_overlay_visual() {
  ...
  Atom actualType;
  int actualFormat;
  if (XGetWindowProperty(fl_display, GI_DESKTOP_WINDOW_ID,
  ...
  if (actualType == overlayVisualsAtom && actualFormat == 32) {
    ...
    for (int i = 0; i < n; i++) {
      ...
      gi_screen_info_t templt;
      templt.visualid = overlayInfo[i].overlay_visual;
      int num;
      gi_screen_info_t *v1=XGetVisualInfo(fl_display, VisualIDMask, &templt, &num);
    ...
    if (v) {
      fl_overlay_visual = v;
      fl_overlay_colormap = 
	XCreateColormap(fl_display, GI_DESKTOP_WINDOW_ID,
			v->visual, AllocNone);
    }
  }
