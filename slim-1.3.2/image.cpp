...
gi_window_id_t
Image::createPixmap( gi_window_id_t win) {
    int i, j;   // loop variables

    const long depth = gi_screen_format();
    //Visual *visual = DefaultVisual(dpy, scr);
    //Colormap colormap = DefaultColormap(dpy, scr);

    gi_window_id_t tmp = gi_create_pixmap_window( win, width, height,
                               depth);
    ...
    gi_image_t *ximage = gi_create_image_with_data(
		width, height,pixmap_data,(gi_format_code_t)depth);

    int entries;
	gi_screen_info_t *visual_info;
	visual_info = gi_default_screen_info();
...
#endif		
	{
            ...
            computeShift(visual_info->redmask, red_left_shift,
                         red_right_shift);
            computeShift(visual_info->greenmask, green_left_shift,
                         green_right_shift);
            computeShift(visual_info->bluemask, blue_left_shift,
                         blue_right_shift);
            ...
            for (j = 0; j < height; j++) {
                for (i = 0; i < width; i++) {
                    ...
                    pixel = (((red << red_left_shift) & visual_info->redmask)
                             | ((green << green_left_shift)
                                & visual_info->greenmask)
                             | ((blue << blue_left_shift)
                                & visual_info->bluemask));

                    gi_image_putpixel(ximage, i, j, pixel);
                }
            }
        }
...
#endif
    gi_gc_ptr_t gc = gi_create_gc(tmp,  NULL);
    //gi_gc_ptr_t gc = gi_create_gc(win,  NULL);
    //XPutImage(dpy, tmp, gc, ximage, 0, 0, 0, 0, width, height);

	gi_draw_image(gc,ximage,0,0);
