...
static fltk::Cursor arrow = {0,GI_CURSOR_ARROW};
static fltk::Cursor cross = {0,GI_CURSOR_CROSS};
static fltk::Cursor wait_c = {0,GI_CURSOR_WAIT};
static fltk::Cursor insert = {0,GI_CURSOR_IBEAM};
static fltk::Cursor hand = {0,GI_CURSOR_HAND};
static fltk::Cursor help = {0,GI_CURSOR_HELP};
static fltk::Cursor move = {0,GI_CURSOR_MOVE};
static fltk::Cursor ns = {0,GI_CURSOR_SIZENS};
static fltk::Cursor we = {0,GI_CURSOR_SIZEWE};
static fltk::Cursor nwse = {0,GI_CURSOR_SIZENWSE};
static fltk::Cursor nesw = {0,GI_CURSOR_SIZENESW};
#if 0
static fltk::Cursor no = {0,0,4};
static fltk::Cursor none = {0,GI_CURSOR_NO};
#else
static fltk::Cursor no = {0,GI_CURSOR_ARROW};
static fltk::Cursor none = {0,GI_CURSOR_ARROW};
#endif
// this probably should be a nicer bitmap:
fltk::Cursor fl_drop_ok_cursor = {0,21};
...
void Widget::cursor(fltk::Cursor* c) const {
  ...
  if (!c) {
    xcursor = GI_CURSOR_ARROW;//None;
    //xcursor = GI_CURSOR_CROSS;//None;
  } else {
    // Figure out the X cursor if not already figured out:
    if (!c->cursor) {
      if (c->fontid) {
		 // TRACE_HERE;
	//c->cursor = gi_load_cursor(xdisplay, (c->fontid-1)*2);
	c->cursor =c->fontid;
	//( (c->fontid));
      } else {

		  c->cursor = GI_CURSOR_HAND;
	...
	//xcursor=gi_create_cursor_image(i->xid,0,0,img);
      }
 #if 0
     uchar r,g,b;
      XColor fgc;
