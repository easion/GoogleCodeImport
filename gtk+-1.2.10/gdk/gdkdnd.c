...
#define PRINTFILE(s)        printf("gdkdnd.c:%s\n", s);
...
struct _GdkDragContextPrivate {
  ...
  gi_window_id_t dest_xid;              /* The last window we looked up */
  gi_window_id_t drop_xid;            /* The (non-proxied) window that is receiving drops */
...
#define PRITFILE(s) printf("gdkdnd.c:%s\n", s)

GdkDragContext *
gdk_drag_context_new        (void)
{
	PRINTFILE("gdk_drag_context_new()");
	return NULL;
}
...
{
	PRINTFILE("gdk_drag_context_ref()");
}

void            
gdk_drag_context_unref (GdkDragContext *context)
{
	PRINTFILE("gdk_drag_context_unref()");
}
...
#endif
{
 	PRINTFILE("gdk_drag_context_find()"); 
  return NULL;
}
...
gdk_window_cache_add (GdkWindowCache *cache,
		      ...
		      gboolean mapped)
{
	PRINTFILE("gdk_window_cache_add()");
}
...
gdk_window_cache_filter (GdkXEvent *xev,
			 ...
			 gpointer   data)
{
	PRINTFILE("gdk_window_cache_filter()");
  return 0;
}
...
{
	PRINTFILE("gdk_window_cache_new()");
  return NULL;
}
...
{
	PRINTFILE("gdk_window_cache_destroy()");
}
...
#else
static gulong 
get_client_window_at_coords_recurse ( gint    x, gint    y)
#endif
{
	PRINTFILE("get_client_window_at_coords_recurse()");
	return 0;
}
...
#endif
{
	PRINTFILE("get_client_window_at_coords()");
	return 0;
}
...
motif_drag_window_filter (GdkXEvent *xevent,
			  ...
			  gpointer data)
{
	PRINTFILE("motif_drag_window_filter()");
	return 0;
}
...
{
	PRINTFILE("motif_find_drag_window()");
	return 0;
}
...
motif_top_level_leave (GdkEvent *event,
		       ...
		       guint32   timestamp)
{
	PRINTFILE("motif_top_level_leave()");
	return 0;
}
...
motif_operation_changed (GdkEvent *event,
			 ...
			 guint32   timestamp)
{
PRINTFILE("motif_operation_changed()");	
  return GDK_FILTER_REMOVE;
}
...
#else
gint
xdnd_send_xevent(gboolean propagate)
#endif
{
	PRINTFILE ("xdnd_send_xevent()");
	return 0;
}
...
{
	PRINTFILE("gdk_drag_get_protocol()");
  return GDK_NONE;
}
...
gdk_drag_find_window (GdkDragContext  *context,
		      ...
		      GdkDragProtocol *protocol)
{
	PRINTFILE("gdk_drag_find_window()");
}
...
{
	PRINTFILE("gdk_window_register_dnd()");
}
...
{
	PRINTFILE("gdk_drag_get_selection()");
    return GDK_NONE;
}
