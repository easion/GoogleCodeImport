...
typedef struct _GdkFontGix  GdkFontGix;
...
static GdkFont *
gdk_gix_bogus_font (gint height)
{
  GdkFont         *font;
  GdkFontGix *private;

  private = g_new0 (GdkFontGix, 1);
...
gdk_font_from_description_for_display (GdkDisplay * display,PangoFontDescription *font_desc)
{
  ...
  return gdk_gix_bogus_font (PANGO_PIXELS (size));
}
...
_gdk_font_strlen (GdkFont     *font,
                  const gchar *str)
{
  GdkFontGix *font_private;
  gint length = 0;

  g_return_val_if_fail (font != NULL, -1);
  g_return_val_if_fail (str != NULL, -1);

  font_private = (GdkFontGix*) font;
...
{
  const GdkFontGix *font_private;

  g_return_val_if_fail (font != NULL, 0);

  font_private = (const GdkFontGix*) font;
...
gdk_font_equal (const GdkFont *fonta,
                const GdkFont *fontb)
{
  const GdkFontGix *privatea;
  const GdkFontGix *privateb;

  g_return_val_if_fail (fonta != NULL, FALSE);
  g_return_val_if_fail (fontb != NULL, FALSE);

  privatea = (const GdkFontGix*) fonta;
  privateb = (const GdkFontGix*) fontb;
