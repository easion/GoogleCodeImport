...
static void figure_out_visual() {
  beenhere = 1;
  open_display();
  if (!xvisual->redmask || !xvisual->greenmask || !xvisual->bluemask){
#if USE_COLORMAP
    fl_redmask = 0;
  ...
  for (i = 0, m = 1; m; i++, m<<=1) if (xvisual->redmask & m) break;
  for (j = i; m; j++, m<<=1) if (!(xvisual->redmask & m)) break;
  fl_redshift = j-8;
  fl_redbits = j-i; if (fl_redbits > 8) fl_redbits = 8;
  fl_redmask = 0xff << (8-fl_redbits);

  for (i = 0, m = 1; m; i++, m<<=1) if (xvisual->greenmask & m) break;
  for (j = i; m; j++, m<<=1) if (!(xvisual->greenmask & m)) break;
  fl_greenshift = j-8;
  fl_greenbits = j-i; if (fl_greenbits > 8) fl_greenbits = 8;
  fl_greenmask = 0xff << (8-fl_greenbits);

  for (i = 0, m = 1; m; i++, m<<=1) if (xvisual->bluemask & m) break;
  for (j = i; m; j++, m<<=1) if (!(xvisual->bluemask & m)) break;
  fl_blueshift = j-8;
...
void fltk::setcolor(Color i) {
  ...
  //printf("setcolor %x,current_xpixel=%x\n",i,current_xpixel);

  //TRACE_HERE;
  gi_set_gc_foreground_pixel( gc,current_xpixel);
}
...
void fltk::line_style(int style, float width, const char* dashes) {
  ...
  if (ndashes) {
	  gi_set_dashes( gc, 0, ndashes, dashes);
  }

  static int Cap[4] = {GI_GC_CAP_BUTT, GI_GC_CAP_BUTT, GI_GC_CAP_ROUND, GI_GC_CAP_PROJECTING};
  static int Join[4] = {GI_GC_JOIN_MITER, GI_GC_JOIN_MITER, GI_GC_JOIN_ROUND, GI_GC_JOIN_BEVEL};
  gi_set_line_attributes( gc, int(width+.5),
		     ndashes ? GI_GC_LINE_ON_OFF_DASH : GI_GC_LINE_SOLID,
