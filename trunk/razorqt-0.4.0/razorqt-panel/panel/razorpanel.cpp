 ...
 ************************************************/
extern "C" int gi_borderless_window(gi_window_id_t window, int borderless);
...
/*
	gi_window_id_t w = internalWinId();
	if(w){
	gi_borderless_window(w,TRUE);
	printf("gi_borderless_window for window %d\n", w);
	}
	else{
	printf("gi_borderless_window for window %d failed\n", w);
	}
*/

    setObjectName("RazorPanel(gix)");
}
...
void RazorPanelPrivate::realign()
{
    ...
    */
    ...
    XfitMan xf = xfitMan();
    gi_window_id_t wid = q->effectiveWinId();
 ...
 ************************************************/
void RazorPanel::pfEventFilter(gi_msg_t* event)
{
    Q_D(RazorPanel);
    foreach (RazorPanelPlugin* plugin, d->mPlugins)
    {
        plugin->pfEventFilter(event);
    }
}
