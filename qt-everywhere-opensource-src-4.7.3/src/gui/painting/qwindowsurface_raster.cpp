    ...
    : QWindowSurface(window), d_ptr(new QRasterWindowSurfacePrivate)
{
	d_ptr->gc = NULL;
#ifdef Q_WS_GIX
    d_ptr->gc = gi_create_gc( window->handle(), NULL);
...
void QRasterWindowSurface::beginPaint(const QRegion &rgn)
{
	...
	|| (defined(Q_WS_WIN) && !defined(Q_WS_WINCE))
	...
	gi_window_id_t hd, gi_gc_ptr_t gc,  gi_screen_info_t *si)
{
        ...
        || (image_byte_order == LSBFirst && bgr_layout))
    {
		...
		qDebug("draw non xrgb pixel\n");
        xi = gi_create_image_with_param(
			w, h,GI_RENDER_x8r8g8b8, (char *) im.bits(), im.bytesPerLine() );

    } else {
        xi = gi_create_image_with_param(w, h,GI_RENDER_x8r8g8b8,
			(char *) image.scanLine(rect.y())+rect.x()*sizeof(uint),
			image.bytesPerLine());		
    }
	...
	if (rv < 0)
	{
		perror("gi_gc_attch_window #1");
	}

	rv = gi_bitblt_image(gc, 0, 0,
				w, h, (gi_image_t*)xi,pos.x(), pos.y());
	if (rv < 0)
	{
	}

    xi->rgba = 0; // QImage owns these bits
    gi_destroy_image(xi);
}
...
//     CGLayerRelease(layer);
    ...
    if (widget->window() != window()) {
        gi_destroy_gc( d_ptr->gc);
        d_ptr->gc = gi_create_gc(widget->handle(), 0);
    }
    ...
    if (wrgn.rectCount() != 1) {
        int num;
        gi_boxrec_t *rects = (gi_boxrec_t *)qt_getClipRects(wrgn, num);
        gi_set_gc_clip_rectangles( d_ptr->gc, rects, num);
    }
    ...
    QRect br = rgn.boundingRect().translated(offset);
    {
        ...
        /*if (src.format() != QImage::Format_RGB32 || widget->x11Info().depth() < 24) {
            ...
            data->fromImage(sub_src, Qt::NoOpaqueDetection);
            QPixmap pm = QPixmap(data);
            XCopyArea( pm.handle(), widget->handle(), d_ptr->gc, 0 , 0 , br.width(), br.height(), wbr.x(), wbr.y());
        } else*/ {
            // qpaintengine_x11.cpp

			

			//gi_image_t *dstimg = src.toGixBitmap();
			if (!d_ptr->gc)
			{
			...
			//	(gi_image_t*)dstimg,pos.x(), pos.y());

			//gi_destroy_image(dstimg);
