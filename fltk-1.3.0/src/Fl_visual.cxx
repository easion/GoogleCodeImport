...
static int test_visual(gi_screen_info_t& v, int flags) {
...
int Fl::visual(int flags) {
  ...
  // get all the visuals:
  gi_screen_info_t vTemplate;
  int num;
  gi_screen_info_t *visualList = XGetVisualInfo(fl_display, 0, &vTemplate, &num);
  // find all matches, use the one with greatest depth:
  gi_screen_info_t *found = 0;
  ...
  fl_visual = found;
  fl_colormap = XCreateColormap(fl_display, GI_DESKTOP_WINDOW_ID,
