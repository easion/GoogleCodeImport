...
static void screen_init() {
  ...
  screens[0].x      = 0;
  screens[0].y      = 0;
  screens[0].w  = gi_screen_width();
  screens[0].h = gi_screen_height();

#define DPIX	75
#define DPIY	75
  int mwidth      = (screens[0].w * 254 + DPIX * 5) / (DPIX * 10);
  int mheight     = (screens[0].h * 254  + DPIY * 5) / (DPIY * 10);


  //int mm = DisplayWidthMM(fl_display, fl_screen);
  dpi[0] = mwidth ? Fl::w()*25.4f/mwidth : 0.0f;
  //mm = DisplayHeightMM(fl_display, fl_screen);
  dpi[1] = mheight ? Fl::h()*25.4f/mheight : dpi[0];  

	//dpi_h[0] = screens[0].width / (s.width/25.4);
   // dpi_v[0] = screens[0].height / (s.height/25.4);
}
