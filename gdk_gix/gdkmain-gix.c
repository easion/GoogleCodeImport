...
/*
 ...
 most stuff in main is moved over to gdkdisplay-gix.c
 I'll move stub functions here that make no sense for gix
 and true globals
 Michael Emmel
*/
...
_gdk_windowing_exit (void)
{
  ...
  //_gdk_display->layer->Release (_gdk_display->layer);
   ...
   //_gdk_display->gix->Release (_gdk_display->gix);
...
//FIXME DPP
GdkWindow *
gdk_gix_other_event_window (GdkWindow    *window,
                                 GdkEventType  type)
{
  guint32    evmask;
  GdkWindow *w;
...
GdkWindow *
gdk_gix_pointer_event_window (GdkWindow    *window,
                                   GdkEventType  type)
{
  guint            evmask;
  ...
  gdk_gix_mouse_get_info (NULL, NULL, &mask);

  if (_gdk_gix_pointer_grab_window && !_gdk_gix_pointer_grab_owner_events )
    {
      evmask = _gdk_gix_pointer_grab_events;
      ...
      if (evmask & type_masks[type]) {
        ...
        if( _gdk_gix_pointer_grab_owner_events ) {
        return _gdk_gix_pointer_grab_window;
        }else {
                GdkWindowObject *obj= GDK_WINDOW_OBJECT(window);
                while (obj != NULL && 
                    obj != GDK_WINDOW_OBJECT(_gdk_gix_pointer_grab_window)) {
                    obj = (GdkWindowObject *)obj->parent;
                }
                if( obj ==GDK_WINDOW_OBJECT(_gdk_gix_pointer_grab_window) ) {
                    return  window;
                }else {
...
GdkWindow *
gdk_gix_keyboard_event_window (GdkWindow    *window,
                                    GdkEventType  type)
{
  guint32    evmask;
  GdkWindow *w;
  ...
  if (_gdk_gix_keyboard_grab_window &&
      !_gdk_gix_keyboard_grab_owner_events)
    {
...
GdkEvent *
gdk_gix_event_make (GdkWindow    *window,
                         GdkEventType  type)
{
  GdkEvent *event    = gdk_event_new (GDK_NOTHING);
  guint32   the_time = gdk_gix_get_time ();
  event->any.type       = type;
  event->any.window     = g_object_ref (window); //FIXME DPP 需要重新引用否？
				   ...
				   gboolean    *owner_events)
{
  if (_gdk_gix_pointer_grab_window)
    {
      if (grab_window)
        *grab_window = (GdkWindow *)_gdk_gix_pointer_grab_window;
      if (owner_events)
        *owner_events = _gdk_gix_pointer_grab_owner_events;
				    ...
				    gboolean    *owner_events)
{
  if (_gdk_gix_keyboard_grab_window)
    {
      if (grab_window)
        *grab_window = (GdkWindow *) _gdk_gix_keyboard_grab_window;
      if (owner_events)
        *owner_events = _gdk_gix_keyboard_grab_owner_events;
...
gdk_keyboard_grab (GdkWindow *window,
                   ...
                   guint32    time) 
{
	return gdk_gix_keyboard_grab(gdk_display_get_default(),
			window,
			owner_events,
			time);
                  ...
                  guint32       time)
{
  ...
  return gdk_gix_pointer_grab (window,
                                    owner_events,
                                    event_mask,
                                    confine_to,
                                    cursor,
...
gdk_pointer_grab (GdkWindow *     window,
                  ...
                  guint32         time)
{
	return gdk_gix_pointer_grab(window, owner_events,event_mask,
