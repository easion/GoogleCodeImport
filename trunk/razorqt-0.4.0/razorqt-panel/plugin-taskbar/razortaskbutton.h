...
class RazorTaskButton : public QToolButton
{
    Q_OBJECT
public:
    explicit RazorTaskButton(const gi_window_id_t window, QWidget *parent = 0);
    virtual ~RazorTaskButton();

    bool isAppHidden() const;
    bool isApplicationActive() const;
    gi_window_id_t windowId() const { return mWindow; }
...
typedef QHash<gi_window_id_t,RazorTaskButton*> RazorTaskButtonHash;
