...
bool QETWidget::translatePropertyEvent(const gi_msg_t *event)
{
    ...
    if (event->xproperty.atom == ATOM(_KDE_NET_WM_FRAME_STRUT)) {
        ...
        if (event->xproperty.state == PropertyNewValue) {
            e = XGetWindowProperty(event->ev_window, ATOM(_KDE_NET_WM_FRAME_STRUT),
        ...
        if (event->xproperty.state == PropertyNewValue) {
            ...
            // possible NET states...
            e = XGetWindowProperty(X11->display, event->ev_window, ATOM(_NET_WM_STATE), 0, 1024,
                                   FALSE, XA_ATOM, &ret, &format, &nitems, &after, &data);

            if (e == Success && ret == XA_ATOM && format == 32 && nitems > 0) {
                gi_atom_id_t *states = (gi_atom_id_t *) data;
