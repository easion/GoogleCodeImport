...
#elif defined(USE_GI)
Window* fltk::find(gi_window_id_t xid)
#elif defined(_WIN32)
...
# error
  ...
  if (grab && widget) {
    ...
    if (window) {
		...
		gi_grab_keyboard( xid(window), true, 0, 0 );

		TRACE_HERE;
		grab_ = true;
	gi_grab_pointer(xid(window),TRUE,FALSE,GI_MASK_BUTTON_DOWN,0,GI_BUTTON_L|GI_BUTTON_R|GI_BUTTON_M);
	}
...
//default: break;
  ...
  if (event) {
    ...
    if (to && to->send(event)) {dnd_flag = false; 
	return true;}
    dnd_flag = false;
  }
