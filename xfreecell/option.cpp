...
Option::Option()
  : mainCon(300, 200), speedCon(300, 50), speedLabel("Speed"), speedTF(80), 
...
void Option::buttonAction(const gi_msg_t& ev, void*)
{
  exitPressed = true;
  //if (ev.xany.window == okButton.window()) okPressed = true;
  if (ev.ev_window == okButton.window()) okPressed = true;
}
...
void Option::waitForEvent()
{
  ...
  while (!exitPressed) {
    ...
    //win = NSWindow::windowToNSWindow(ev.xany.window);		// FIXME
