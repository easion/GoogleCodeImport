...
static gi_image_t xi;	// template used to pass info to X
...
static void figure_out_visual() {
  ...
  int byte_order = !WORDS_BIGENDIAN;
  ...
  xi.format = fl_visual->format;
  int bits_per_pixel = GI_RENDER_FORMAT_BPP(fl_visual->format);

 // gi_image_surface_init(&xi.s, (fl_visual->format));

  if (bits_per_pixel & 7) bytes_per_pixel = 0; // produce fatal error
  else bytes_per_pixel = bits_per_pixel/8;

  unsigned int n = 0;//pfv->scanline_pad/8;
  //if (pfv->scanline_pad & 7 || (n&(n-1)))
  //  Fl::fatal("Can't do scanline_pad of %d",pfv->scanline_pad);
  if (n < sizeof(STORETYPE)) n = sizeof(STORETYPE);
  scanline_add = n-1; //FIXME DPP
  scanline_mask = -n;
...
#  if USE_COLORMAP
  ...
  if (!fl_visual->redmask)
    Fl::fatal("Can't do %d bits_per_pixel colormap",bits_per_pixel);
#  endif
...
#  endif
  ...
  case 3:
    if (byte_order) {rs = 16-rs; gs = 16-gs; bs = 16-bs;}
    if (rs == 0 && gs == 8 && bs == 16) {
  ...
  default:
    Fl::fatal("Can't do %d bits_per_pixel",bits_per_pixel);
  }
...
#  endif
    ...
    xi.rgba = (gi_color_t *)(buf+delta*dx+linedelta*dy);
    xi.pitch = linedelta;
    ...
    xi.rgba = (gi_color_t *)buffer;
    xi.pitch = linesize*sizeof(STORETYPE);
    if (buf) {
  ...
  gi_gc_attch_window(fl_gc,fl_window);
	gi_bitblt_image(fl_gc, 0, 0, w, k, &xi, X+dx, Y+dy+j-k);
      }
    } else {
...
void fl_rectf(int x, int y, int w, int h, uchar r, uchar g, uchar b) {
  if (GI_RENDER_FORMAT_BPP(fl_visual->format) > 16) {
    fl_color(r,g,b);
    fl_rectf(x,y,w,h);
  } else {
    uchar c[3];
