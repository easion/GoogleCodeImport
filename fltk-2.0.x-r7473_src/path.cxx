...
void fltk::drawpoints() {
  ...
  if (numpoints > 0) XDrawPoints(xdisplay, xwindow, gc, xpoint, numpoints, 0);
    ...
    gi_set_pixel(gc, xpoint[i].x, xpoint[i].y, current_xpixel);
#elif defined(_WIN32)
  for (int i=0; i<numpoints; i++)
    ...
    int mode)
{
	...
	if (npoints < 1)
		return 1;
	if (npoints == 1) {
		gi_set_pixel( gc, points->x, points->y);
		return 1;
	}

	gi_gc_attch_window(gc,d);

	//printf("%s(): (%d) -%d\n",__FUNCTION__,d, npoints);
	...
	while (--npoints > 0) {
		...
		++points;
		gi_draw_line( gc, beg.x, beg.y, end.x, end.y);
		beg = end;
	}
    return 1;
}
...
void fltk::strokepath() {
  ...
  if (circle_type) {
    ...
    if (r.w() < 2 || r.h() < 2) {
      if (!r.empty())
	gi_fill_rect(  gc, r.x(), r.y(), r.w(), r.h());
    } else {
      gi_draw_arc(xwindow,  gc, r.x(), r.y(), r.w()-1, r.h()-1, A, B);
    }
  }
  int loop_start = 0;
  for (int n = 0; n < loops; n++) {
    int loop_size = loop[n];
    gi_draw_lines( xwindow, gc, xpoint+loop_start, loop_size, 0);
    loop_start += loop_size;
  }
  int loop_size = numpoints-loop_start;
  if (loop_size > 1)
...
if (circle_type) {
    ...
    int B = int(circle_end*64)-A;
    const Rectangle& r = circle;
    gi_set_arcmode( gc, circle_type == PIE ? 1 : 0);
    if (r.w() < 2 || r.h() < 2) {
      if (!r.empty())
	gi_fill_rect(  gc, r.x(), r.y(), r.w(), r.h());
    } else {
      gi_fill_arc( xwindow, gc, r.x(), r.y(), r.w()-1, r.h()-1, A, B);
      gi_draw_arc( xwindow, gc, r.x(), r.y(), r.w()-1, r.h()-1, A, B);
    }
  }
  if (loops) closepath();
  if (numpoints > 2) {
    ...
    gi_fill_polygon(  gc, xpoint, numpoints, 0, 0);
  }
