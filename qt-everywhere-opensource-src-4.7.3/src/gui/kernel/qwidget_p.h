...
struct QTLWExtra {
	...
	//uint flags : 31;
    WId parentWinId; // parent window Id (valid after reparenting)
    uint winFlags; // window id that contains user-time timestamp when WM supports a _NET_WM_USER_TIME_WINDOW atom
    QPoint fullScreenOffset;
	gi_image_t* icon_big;
	gi_image_t* icon_small;
...
#endif
#elif defined(Q_WS_X11) || defined(Q_WS_GIX)// <--------------------------------------------------------- X11
