...
static gi_ufont_t* load_xfont_for_xft2(Fl_Graphics_Driver *driver) {
  gi_ufont_t* xgl_font = 0;
...
extern gi_screen_info_t* fl_overlay_visual;
#endif
...
static gi_window_id_t draw_window;
#if USE_OVERLAY
static XftDraw* draw_overlay;
static gi_window_id_t draw_overlay_window;
#endif

void fl_destroy_xft_draw(gi_window_id_t id) {
  if (id == draw_window)
...
void Fl_Xlib_Graphics_Driver::draw(const char *str, int n, int x, int y) {
  ...
  gi_region_ptr_t region = fl_clip_region();
