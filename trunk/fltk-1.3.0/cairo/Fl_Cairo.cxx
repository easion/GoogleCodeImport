...
cairo_t * Fl::cairo_make_current(Fl_Window* wi) {
    ...
    if (fl_gc==Fl::cairo_state_.gc() && fl_xid(wi) == (gi_window_id_t) Fl::cairo_state_.window())
	return Fl::cairo_cc();
...
# if defined(USE_X11)
    //return cairo_gix_surface_create(fl_display, fl_window, fl_visual->visual, W, H);
    return cairo_gix_surface_create( fl_window);
# elif   defined(WIN32)
...
# error Cairo is not supported under this platform.
    ...
    if (gc==Fl::cairo_state_.gc() && fl_window== (gi_window_id_t) Fl::cairo_state_.window())
	return Fl::cairo_cc();
