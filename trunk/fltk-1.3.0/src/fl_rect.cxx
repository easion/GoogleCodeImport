...
void Fl_Graphics_Driver::rect(int x, int y, int w, int h) {
  ...
  if (!clip_to_short(x, y, w, h))
    gi_draw_rect( fl_gc, x, y, w-1, h-1);
...
#if defined(USE_GIX)
  errno = 0;
  err1 = gi_gc_attch_window(fl_gc,fl_window);
  if (!clip_to_short(x, y, w, h)){
    err2 = gi_fill_rect(  fl_gc, x, y, w, h);
  }
#elif defined(USE_X11)
  if (!clip_to_short(x, y, w, h))
...
#if defined(USE_GIX)
  gi_gc_attch_window(fl_gc,fl_window);
  gi_draw_line(  fl_gc, clip_x(x), clip_x(y), clip_x(x1), clip_x(y));
#elif defined(USE_X11)
...
#if defined(USE_GIX)
  gi_gc_attch_window(fl_gc,fl_window);
  gi_draw_line(  fl_gc, clip_x(x), clip_x(y), clip_x(x), clip_x(y1));
#elif defined(USE_X11)
...
void Fl_Graphics_Driver::polygon(int x, int y, int x1, int y1, int x2, int y2) {
  ...
  p[3].x = x;  p[3].y = y;
  gi_gc_attch_window(fl_gc,fl_window);
  gi_fill_polygon(  fl_gc, p, 3, GI_SHAPE_Convex, 0);
  gi_draw_lines(  fl_gc, p, 4);
#elif defined (USE_X11)
  p[3].x = x;  p[3].y = y;
...
void Fl_Graphics_Driver::polygon(int x, int y, int x1, int y1, int x2, int y2, int x3, int y3) {
  ...
  gi_gc_attch_window(fl_gc,fl_window);
  p[4].x = x;  p[4].y = y;
  gi_fill_polygon(  fl_gc, p, 4, GI_SHAPE_Convex, 0);
  gi_draw_lines(  fl_gc, p, 5);
#elif defined(USE_X11)
  p[4].x = x;  p[4].y = y;
...
#if defined(USE_GIX)
  if (r) gi_set_gc_clip_rectangles( fl_gc,  r->rects, r->numRects);
  else gi_set_gc_clip_rectangles( fl_gc,  NULL, 0);
#elif defined(USE_X11)
