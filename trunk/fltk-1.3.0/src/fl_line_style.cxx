...
void Fl_Graphics_Driver::line_style(int style, int width, char* dashes) {
  ...
  static int Cap[4] = {GI_GC_CAP_BUTT, GI_GC_CAP_BUTT, 
	  GI_GC_CAP_ROUND, GI_GC_CAP_PROJECTING};
  static int Join[4] = {GI_GC_JOIN_MITER, GI_GC_JOIN_MITER,
	  GI_GC_JOIN_ROUND, GI_GC_JOIN_BEVEL};
  gi_set_line_attributes( fl_gc, width, 
		     ndashes ? GI_GC_LINE_ON_OFF_DASH : GI_GC_LINE_SOLID,
		     Cap[(style>>8)&3], Join[(style>>12)&3]);
  if (ndashes) gi_set_dashes( fl_gc, 0, ndashes, dashes);
#elif defined(WIN32)
