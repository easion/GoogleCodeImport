...
#define INTL_DEF(_encoding, _locale, _charset, _is_char_func, _fontname) \
	{ _encoding, _locale, _is_char_func },
//#include "../../microwin/src/include/intl.def"
#undef INTL_DEF
};

static void _gdk_switch_encoding (int locale);
static void _gdk_set_langfunc (int en);

int _gdk_get_encoding ();
int _gdk_get_encoding_width ();
void _gdk_set_encoding_width (int width);
void _gdk_set_encoding_l (int en);   /* Just set the variable
					in local (after broadcast) */
void _gdk_set_encoding (int en);     /* ask the server to change the
					encoding => broadcast */
static int encoding = 0;
static int encoding_width = 0;
gboolean (*is_asian_char)(guchar c1, guchar c2) = NULL;
...
#endif /* USE_XIM */
#define PRINTFILE(s) printf("gdkim.c:%s()\n",s);
/*
 *--------------------------------------------------------------
 * gdk_set_locale
 *
...
gdk_set_locale (void)
{
	...
	if ( (current_locale = setlocale(LC_ALL, "")) == NULL)
		g_warning("locale not supported by C library");
	else
	if (strcmp("C", current_locale) && strcmp("POSIX", current_locale))
	{
		gdk_use_mb = TRUE;
	}
...
#if 0 //dpp
	/*
	 * This function allow application like browser to switch the
         * system's language encoding.
	 */
	if ((current_locale = getenv("CJK_LANG")) != NULL) {
		for (i = 0; intl_table[i].encoding != LANG_ENCODING_UNKNOWN;i++)
		{
			if (!strncasecmp(current_locale, intl_table[i].locale,
					 strlen(intl_table[i].locale)))
			{
				_gdk_switch_encoding(intl_table[i].encoding);
				break;
			}
		}
	}
...
gdk_ic_attr_new (void)
{
  g_warning("%s not supported",__FUNCTION__);
  return 0;
}
...
gdk_wcstombs (const GdkWChar *src)
{
	...
	mbstr = g_new (gchar, length * 2 + 1);

	wi = 0;
	for (i = 0; i < length; i++) {
		if (src[i] > 0xff) {
			mbstr[wi++] = src[i] >> 8;
			mbstr[wi++] = src[i] & 0x00ff;
		} else {
			mbstr[wi++] = src[i];
		}
	}
...
gdk_mbstowcs (GdkWChar *dest, const gchar *src, gint dest_max)
{
	...
	for (i = 0, wi = 0; i < dest_max && src[i]; i++, wi++) {
		guchar ch = src[i];

		if (src[i+1] != '\0' && is_asian_char != NULL
		 && (*is_asian_char)(src[i], src[i+1]))
		{
			dest[wi] = (ch << 8) | (guchar)src[i+1];
			i = i + 1;
		} else {
			dest[wi] = src[i] & 0x00ff;
		}
	}
...
{
        return encoding_width;
}

void
_gdk_set_encoding_width (int width)
{
        encoding_width = width;
}

void
_gdk_set_encoding_l (int en)
{
        encoding = en;
}

void
_gdk_set_encoding (int en)
{
	//GrSetEncoding(en);
	encoding = en;
}
...
static void
_gdk_switch_encoding (int locale)
{
	int dummy_locale, width;

	if (locale != _gdk_get_encoding()) {
		_gdk_set_encoding(locale);
		//GrGetEncoding(&dummy_locale, &width);
		_gdk_set_encoding_width(width);
	}
	_gdk_set_langfunc(locale);
}

static void
_gdk_set_langfunc (int encoding)
{
	int i;

	is_asian_char = NULL;
	for (i = 0; i < sizeof intl_table / sizeof intl_table[0]; i++) {
		if (encoding == intl_table[i].encoding) {
			is_asian_char = intl_table[i].is_char_func;
			break;
		}
        }
}
 ...
 */
gboolean
is_simp_chinese_char (guchar c1, guchar c2)
{
	if (c1 >= 0xa1 && c1 <= 0xfe
         && c2 >= 0xa1 && c2 <= 0xfe)
		return TRUE;
 ...
 */
gboolean
is_trad_chinese_char (guchar c1, guchar c2)
{
	if ((c1 >= 0xa1 && c1 <= 0xfe) &&
	   ((c2 >= 0x40 && c2 <= 0x7e) ||
	   ((c2 >= 0xa1 && c2 <= 0xfe))))
 ...
 */
gboolean
is_japanese_char (guchar c1, guchar c2)
{
	return is_simp_chinese_char(c1, c2);
}
 ...
 */
gboolean
is_korean_char (guchar c1, guchar c2)
{
	return is_simp_chinese_char(c1, c2);
}
