...
void Fl_Graphics_Driver::end_points() {
  ...
  if (n>1) gi_draw_points( fl_gc, p, n);
...
#if defined(USE_GIX)
	gi_gc_attch_window(fl_gc,fl_window);
  if (n>1) gi_draw_lines(  fl_gc, p, n);
#elif defined(USE_X11)
...
#if defined(USE_GIX)
	gi_gc_attch_window(fl_gc,fl_window);
  if (n>2) gi_fill_polygon(  fl_gc, p, n, GI_SHAPE_Convex, 0);
#elif defined(USE_X11)
...
#if defined(USE_GIX)
	gi_gc_attch_window(fl_gc,fl_window);
  if (n>2) gi_fill_polygon(  fl_gc, p, n, 0, 0);
#elif defined(USE_X11)
...
#if defined(USE_GIX)
	gi_gc_attch_window(fl_gc,fl_window);
  (what == POLYGON ? gi_fill_arc : gi_draw_arc)
    (fl_window, fl_gc, llx, lly, w, h, 0, 360*64);
#elif defined(USE_X11)
