...
static void figure_out_visual() {
  beenhere = 1;
  if (!fl_visual->redmask || !fl_visual->greenmask || !fl_visual->bluemask){
#  if USE_COLORMAP
    fl_redmask = 0;
    return;
#  else
...
#  endif
  ...
  for (i = 0, m = 1; m; i++, m<<=1) if (fl_visual->redmask & m) break;
  for (j = i; m; j++, m<<=1) if (!(fl_visual->redmask & m)) break;
  fl_redshift = j-8;
  fl_redmask = (j-i >= 8) ? 0xFF : 0xFF-(255>>(j-i));

  for (i = 0, m = 1; m; i++, m<<=1) if (fl_visual->greenmask & m) break;
  for (j = i; m; j++, m<<=1) if (!(fl_visual->greenmask & m)) break;
  fl_greenshift = j-8;
  fl_greenmask = (j-i >= 8) ? 0xFF : 0xFF-(255>>(j-i));

  for (i = 0, m = 1; m; i++, m<<=1) if (fl_visual->bluemask & m) break;
  for (j = i; m; j++, m<<=1) if (!(fl_visual->bluemask & m)) break;
  fl_blueshift = j-8;
...
gi_screen_info_t* fl_overlay_visual;
