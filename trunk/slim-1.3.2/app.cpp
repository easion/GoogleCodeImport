...
void App::Run() {
    ...
    if (testing) {
        gi_window_id_t RealRoot = GI_DESKTOP_WINDOW_ID;
        Root = gi_create_window(RealRoot, 0, 0, 1280, 1024, GI_RGB( 128, 128  , 128   ), GI_FLAGS_NON_FRAME);
        gi_show_window( Root);
        //XFlush(Dpy);
    } else {
        blankScreen();
    ...
    // Create panel
    LoginPanel = new Panel( Root, cfg, themedir);
...
void App::HideCursor() {
    if (cfg->getOption("hidecursor") == "true") {
        ...
        black.green=0;
        black.blue=0;
        cursor=XCreatePixmapCursor(cursorpixmap,cursorpixmap,&black,&black,0,0);
        XDefineCursor(Root,cursor);
#endif
	}
}
...
void App::Console() {
    ...
    int fontx = 9;
    int fonty = 15;
    int width = (gi_screen_width() - (posx * 2)) / fontx;
    int height = (gi_screen_height() - (posy * 2)) / fonty;
...
void App::KillAllClients(BOOL top) {
    gi_window_id_t dummywindow;
    gi_window_id_t *children;
    int nchildren;
    unsigned int i;
    ...
    nchildren = 0;
    gi_query_child_tree( Root,  &dummywindow, &children, &nchildren);
#if 0
    if(!top) {
        for(i=0; i<nchildren; i++) {
            if(gi_get_window_info( children[i], &attr) && (attr.map_state == IsViewable))
...
{
    gi_gc_ptr_t gc = gi_create_gc( Root, 0);
    gi_set_gc_foreground( gc, GI_RGB(0,0,0));
    gi_fill_rect(  gc, 0, 0,
                   gi_screen_width(),
                   gi_screen_height());
    //XFlush(Dpy);
...
void App::setBackground(const string& themedir) {
    ...
    if (loaded) {
        ...
        if (bgstyle == "stretch") {
            image->Resize(gi_screen_width(), gi_screen_height());
        } else if (bgstyle == "tile") {
            image->Tile(gi_screen_width(), gi_screen_height());
        } else if (bgstyle == "center") {
            string hexvalue = cfg->getOption("background_color");
            hexvalue = hexvalue.substr(1,6);
            image->Center(gi_screen_width(), gi_screen_height(),
                        hexvalue.c_str());
        ...
        gi_window_id_t p = image->createPixmap( Root);
        gi_set_window_pixmap(Root, p, 0);
    }
