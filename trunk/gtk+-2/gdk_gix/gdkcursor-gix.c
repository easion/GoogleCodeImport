  ...
  if (!cursor)
    {
      ...
      temp =gi_create_image_depth(width,height,GI_RENDER_a8r8g8b8);
                            ...
                            gint            y)
{
  ...
  GdkDrawableImplGix *mask_impl;
  gi_image_t*        shape = NULL;

  g_return_val_if_fail (GDK_IS_PIXMAP (source), NULL);
  g_return_val_if_fail (GDK_IS_PIXMAP (mask), NULL);

  impl = GDK_DRAWABLE_IMPL_GIX (GDK_PIXMAP_OBJECT (source)->impl);
  mask_impl = GDK_DRAWABLE_IMPL_GIX (GDK_PIXMAP_OBJECT (mask)->impl);
  ...
  WARING_MSG; //FIXME
  gi_get_window_buffer(impl->window_id, 0,0,width, height,
	  shape->pitch<<16 , GI_RENDER_a8r8g8b8, shape->rgba);
...
_gdk_cursor_destroy (GdkCursor *cursor)
{
  ...
  private = (GdkCursorGix *) cursor;
  if (private->shape_img)
  {
	  gi_destroy_image(private->shape_img);
