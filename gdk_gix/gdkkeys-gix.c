		...
		gint          *n_entries)
{
	...
	index = (hardware_keycode - gix_min_keycode) * 4;

	if (gix_keymap && (hardware_keycode >= gix_min_keycode && 
				hardware_keycode <= gix_max_keycode))
	{
		for (i = 0; i < 4; i++)
			if (gix_keymap[index + i] != GDK_VoidSymbol)
				num++;
	}
		...
		*keys = g_new (GdkKeymapKey, num);
		...
		for (i = 0, j = 0; num > 0 && i < 4; i++)
			if (gix_keymap[index + i] != GDK_VoidSymbol)
			{
				...
				(*keys)[j].level   = j % 2;
				(*keys)[j].group   = j > G_MODIFIERS_LSHIFT ? 1 : 0;
				j++;
			}
	}
	if (keyvals)
		...
		gint          *n_keys)
{
	...
	for (i = gix_min_keycode;
			gix_keymap && i <= gix_max_keycode;
			i++)
	{
		gint index = i - gix_min_keycode;

		for (j = 0; j < 4; j++)
		{
			if (gix_keymap[index * 4 + j] == keyval)
				append_keymap_key (retval, i, j % 2, j > G_MODIFIERS_LSHIFT ? 1 : 0);
		}
	}
...
{
	if (gix_keymap &&
			(keycode >= gix_min_keycode && keycode <= gix_max_keycode) &&
			(group == 0 || group == 1))
	{
		gint index = (keycode - gix_min_keycode) * 4;
		gint i =     (state & GDK_SHIFT_MASK) ? 1 : 0;

		if (gix_keymap[index + i + 2 * group] != GDK_VoidSymbol)
		{
			if (keyval)
				*keyval = gix_keymap[index + i + 2 * group];

			if (group && gix_keymap[index + i] == *keyval)
			{
				if (effective_group)
					*effective_group = 0;
			...
			if (i && gix_keymap[index + 2 * *effective_group] != *keyval)
                if(consumed_modifiers)
