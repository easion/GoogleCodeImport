...
#if defined(Q_WS_GIX)
	gi_image_t *toGixBitmap(unsigned format = GI_RENDER_x8r8g8b8) const;
	static QPixmap fromGixBitmap(gi_image_t *);
	static QPixmap fromGixPixmap(gi_window_id_t );
	static gi_image_t* to_gix_icon(const QIcon &icon, bool isPointer = false,
                                 int hotX = 0, int hotY = 0,
                                 bool embedRealAlpha = false, bool isMini = true);
#endif
